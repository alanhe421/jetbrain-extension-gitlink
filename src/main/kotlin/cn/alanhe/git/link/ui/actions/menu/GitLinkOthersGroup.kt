...
class GitLinkOthersGroup : DefaultActionGroup() {
    ...
    private fun updateChildActions() {
        ...
        if (settings.copyGitHubMarkdownLinkMenuLevel == MenuLevel.SUBMENU) {
            actionManager.getAction("cn.alanhe.git.link.ui.actions.menu.MarkdownAction.Others")?.let {
                add(it)
                addedCount++
            }
        }
        ...
        if (settings.copyGitHubMarkdownSnippetMenuLevel == MenuLevel.SUBMENU) {
            actionManager.getAction("cn.alanhe.git.link.ui.actions.menu.CopyMarkdownSnippetAction")?.let {
                add(it)
                addedCount++
            }
        }
        ...
        if (settings.copyGitHubSnippetImageMenuLevel == MenuLevel.SUBMENU) {
            actionManager.getAction("cn.alanhe.git.link.ui.actions.menu.CreateGitHubSnippetImageAction")?.let {
                add(it)
                addedCount++
            }
        }
